...
double Regressor::meanSquaredError(const std::vector<double>& actual, const std::vector<double>& predicted) {
    if (actual.size() != predicted.size() || actual.empty()) {
        throw std::invalid_argument("Veri boyutları uyuşmuyor.");
    }
    ...
    // Loop unrolling + vektörleştirme şansı için klasik for
    for (; i + 3 < size; i += 4) {
        sum += std::pow(actual[i] - predicted[i], 2);
        sum += std::pow(actual[i+1] - predicted[i+1], 2);
        sum += std::pow(actual[i+2] - predicted[i+2], 2);
        sum += std::pow(actual[i+3] - predicted[i+3], 2);
    }
