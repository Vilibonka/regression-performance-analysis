...
int main() {
    std::ifstream file("data_quadratic.txt");
    if (!file) {
    ...
    try {
    	...
    	// Sample data regresyon testi.
    	int sampleSize = 5; // Örnek alınacak veri sayısı

    	try {
        	auto [sampledX, sampledY] = reg.sampleData(x, y, sampleSize);
        	auto [sm, sb] = reg.linearRegression(sampledX, sampledY);

        	std::cout << "\nSample veriden regresyon sonucu:\n";
        	std::cout << "Eğim (m): " << sm << "\n";
        	std::cout << "Kesişim (b): " << sb << "\n";

        	std::vector<double> samplePredicted;
        	for (size_t i = 0; i < sampledX.size(); ++i) {
            	samplePredicted.push_back(sm * sampledX[i] + sb);
        	}

        	double sampleMSE = reg.meanSquaredError(sampledY, samplePredicted);
        	std::cout << "Sample veride MSE: " << sampleMSE << "\n";
    	} catch (const std::exception& e) {
